...
contract RockPaperScissors {
    ...
    //Strings are private so that way others address cannot see their choice
    ...
    event winnerChickenDinner (address indexed whoWon, bool winner);
        ...
        playersMadeChoice() // Can disclose the result when choices are made
    {
        ...
        int result = int(uint(states[choiceOfPlayer1][choiceOfPlayer2]));
        if (result == 0) { //if both players draw they both transfer the stake
            player1.transfer(stake); 
            player2.transfer(stake);
        } else if (result == 1) { //If player 2 wins transfers the stake
            player1.transfer(address(this).balance);
            emit winnerChickenDinner(msg.sender, true);
        } else if (result == 2) { //If player 1 wins transfers the stake
            player2.transfer(address(this).balance);
            emit winnerChickenDinner(msg.sender, true);
        }
        
        // Here we reset the data of the game
